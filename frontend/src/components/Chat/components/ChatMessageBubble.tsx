...
const ChatMessageBubble: React.FC<{ message: Message }> = ({ message }) => (
  <Stack direction="row" alignItems="flex-start" spacing={2} className="message-enter">
    <Avatar
      sx={{
        bgcolor: message.type === MessageType.USER ? 'grey.400' : 'primary.main',
        width: 40,
        height: 40,
        fontSize: 18,
        fontWeight: 500,
    ...
    <Box
      sx={{
        maxWidth: '70%',
        bgcolor: message.type === MessageType.USER ? 'primary.main' : 'transparent',
        color: message.type === MessageType.USER ? 'primary.contrastText' : 'text.primary',
        px: 2,
        py: 1.5,
        borderRadius: 2,
